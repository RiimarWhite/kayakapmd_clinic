...
return new class extends Migration
{
    ...
    {
        Schema::create('dd_diag_fecalysis', function (Blueprint $table) {
            ...
            $table->date('dLabDate')->nullable()->comment('YYYY-MM-DD, Date of Laboratory for FECALYSIS ');
            $table->enum('dColor', ['1', '2', '3', '4', '5', '6'])->nullable()->comment('1 - Brown;
                        2 - Black;
                        3 - Red;
                        4 - White/Grey;
                        5 - Yellow;
                        6 - Green;
                        ');
            $table->enum('dConsistency', ['1', '2', '3', '4', '5', '6'])->nullable()->comment('1 - Soft;
  2 - Well-Formed;
  3 - Semi-Formed;
  4 - Watery;
  5 - Mucoid;
 6 - Hard
                        ');
            $table->string('dRbc', 50)->nullable()->comment('Value for RBC (/hpf)');
            $table->string('dWbc', 50)->nullable()->comment('Value for WBC(/hpf)');
            $table->string('dOva', 50)->nullable()->comment('Value for RBC (=/-)');
            $table->string('dParasite', 50)->nullable()->comment('Value for Parasite (=/-)');
            $table->enum('dBlood', ['P', 'A'])->nullable()->comment('P - Present;
                            A - Absent ');
            $table->string('dPusCells', 50)->nullable()->comment('Value for PUS Cells');
    ...
    {
        Schema::dropIfExists('dd_diag_fecalysis');
    }
};
